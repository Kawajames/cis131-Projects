# File Name: Project5FinalSorting.py
...
base_dir = r"C:\Users\Dakota\Desktop\Project1"
...
class Employee(Person):
    ...
    def _get_key(dictionary, value):
        # allow either the descriptive value or the code key
...
def displayEmployeeContact():
    ...
    print("-" * len(header))
    for emp in employeeList:
        print(f"{emp.lastName:15}{emp.firstName:15}{emp.idNumber:<8}"
              f"{emp.emailAddress:30}{emp.phoneNumber:15}")
...
def displayStudentContact():
    ...
    for stu in studentList:
        print(f"{stu.lastName:15}{stu.firstName:15}{stu.idNumber:<8}"
              f"{stu.emailAddress:30}{stu.phoneNumber:15}")
...
def displayAllContacts():
    ...
    for person in employeeList + studentList:
        print(f"{person.lastName:15}{person.firstName:15}{person.idNumber:<8}"
              f"{person.emailAddress:30}{person.phoneNumber:15}")
...
def getHonorRoll():
    ...
    header = f"{'LastName':15}{'FirstName':15}{'ID':8}{'Avg':8}{'Grade':6}"
    print(header)
    print("-" * len(header))
    any_honors = False
    for stu in studentList:
        report = getStudentAcademicReport(stu)
        avg, grade = report[-2], report[-1]
        if avg >= 90:
            any_honors = True
            print(f"{report[0]:15}{report[1]:15}{report[2]:<8}{avg:<8}{grade:<6}")
    if not any_honors:
        print("No students on the honor roll.")

def sortByLastName():
    studentList.sort(key=lambda s: s.lastName)

def sortByID():
    studentList.sort(key=lambda s: s.idNumber)

def displaySortedByLastName():
    print("\nFull Academic Report (Sorted by Last Name)\n")
    sortByLastName()
    displayFullStudentAcademicReport()

def displaySortedByID():
    print("\nFull Academic Report (Sorted by Student ID)\n")
    sortByID()
    displayFullStudentAcademicReport()
...
def createMenu():
    while True:
        ...
        if choice == "1":
            ...
            displayIndividualStudentReport()
        elif choice == "8":
            getHonorRoll()
        elif choice == "9":
